...
#gets replies
@action("get_replies", method="GET")
@action.uses(db, auth.user)
def get_replies():
    post_id = request.params.get("postId")
    replies = db(db.meow.reply_to == post_id).select()
    return dict(replies=replies)

#publishes reply to database
@action("publish_reply", method="POST")
@action.uses(db, auth.user)
def publish_reply():
    content = request.json.get("content")
    author = get_username()
    ...
    db(db.meow.id == post_id).update(num_replies=db.meow.num_replies + 1)

    reply_id = db.meow.insert(author=author, timestamp=timestamp, content=content, reply_to=post_id) # insert the reply into the database

    return dict(content=content, author=author, timestamp=iso_timestamp, id=reply_id)

#gets current username of logged in user
...
#returns recents posts
@action('get_recent_posts')
@action.uses(db, auth.user)
def get_recent_posts():
    recent_posts = db().select(db.meow.ALL, orderby=~db.meow.timestamp)
    return dict(posts=recent_posts)